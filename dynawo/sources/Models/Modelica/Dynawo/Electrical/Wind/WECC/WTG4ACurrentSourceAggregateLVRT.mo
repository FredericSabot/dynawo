...
model WTG4ACurrentSourceAggregateLVRT "WECC Wind Turbine model with a simplified drive train model (dual-mass model) and with a current source as interface with the grid"
  import Modelica;
  import Dynawo;
  import Dynawo.Types;
  ...
  Dynawo.Connectors.ACPower terminal(V(re(start = u0Pu.re), im(start = u0Pu.im)), i(re(start = i0Pu.re), im(start = i0Pu.im))) annotation(
    Placement(visible = true, transformation(origin = {210, -10}, extent = {{10, -10}, {-10, 10}}, rotation = 0), iconTransformation(origin = {100, 8.88178e-16}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  ...
  Dynawo.Electrical.Controls.WECC.Utilities.CurrentReduction currentReduction annotation(
...
equation
  ...
  connect(measurements.terminal2, currentReduction.terminal1) annotation(
    Line(points = {{160, -10}, {172, -10}}, color = {0, 0, 255}));
  connect(currentReduction.terminal2, terminal) annotation(
    Line(points = {{192, -10}, {210, -10}}, color = {0, 0, 255}));
  connect(measurements.UPu, lVRTAggr.u) annotation(
  ...
  annotation(
    ...
    info = "<html>
<p> WTG4ACurrentSource model with possibility to disconnect part of the distributed generation by multiplying the output current by a factor</p></html>"),
  ...
  Diagram(coordinateSystem(grid = {1, 1}, extent = {{-100, -60}, {200, 50}})));
end WTG4ACurrentSourceAggregateLVRT;
