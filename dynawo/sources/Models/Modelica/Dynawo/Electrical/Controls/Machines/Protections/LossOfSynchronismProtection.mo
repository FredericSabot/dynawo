...
model LossOfSynchronismProtection "Loss of synchronism protection for generators"
  ...
  parameter Types.Angle ThetaMin "Angle threshold above which the automaton is activated in rad. It is recommended to use a value that is not to close to (-)pi as the simulator migth skip it.";
  parameter Types.Time tLagAction "Time-lag due to the actual trip action in s";

  RealInput thetaMonitoredPu "Monitored angle in pu (base omegaNom)";
...
equation
  ...
  when thetaMonitoredPu < -abs(ThetaMin) and not(pre(switchOffSignal.value)) then
