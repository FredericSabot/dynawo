...
model DistanceProtectionLineSide1Special "Simple triangular distance relay, disconnects side 1 of the protected line when the apparent impedance in within a protected zone for longer than a set duration. Can no longer disarm once the circuit-breaker signal is sent."
...
public
  ...
  parameter Types.Time SlowCircuitBreakerTime "Time required to open the circuit breaker in seconds, included in 'time delay of trip in zone X'";
  parameter Types.Time FastCircuitBreakerTime "Time required to open the circuit breaker in seconds, included in 'time delay of trip in zone X'";
...
equation
  assert(FastCircuitBreakerTime < T1, "FastCircuitBreakerTime should be included (thus smaller) than the tripping times");
  ...
  */
  ...
  elsewhen not (Z.re <= R1Pu and Z.im <= X1Pu and Z.re > -Z.im) and pre(tThresholdReached1) <> Constants.inf and time - pre(tThresholdReached1) < T1 - FastCircuitBreakerTime then
  ...
  elsewhen not (Z.re <= R2Pu and Z.im <= X2Pu and Z.re > -Z.im) and pre(tThresholdReached2) <> Constants.inf and time - pre(tThresholdReached2) < T2 - SlowCircuitBreakerTime then
  ...
  elsewhen not (Z.re <= R3Pu and Z.im <= X3Pu and Z.re > -Z.im) and pre(tThresholdReached3) <> Constants.inf and time - pre(tThresholdReached3) < T3 - SlowCircuitBreakerTime then
  ...
  elsewhen not (Z.re <= R4Pu and Z.im <= X4Pu and Z.re > -Z.im) and pre(tThresholdReached4) <> Constants.inf and time - pre(tThresholdReached4) < T4 - FastCircuitBreakerTime then
  ...
  */
  ...
  when time - tThresholdReached2 >= T2 then
...
end DistanceProtectionLineSide1Special;
